...
submitBtn.addEventListener('click', () => {
  ...
  if (photos.length < 3) {
    alert('Please take 3 selfies');
    return;
  }
  // Persist locally (no DB): localStorage for demo
  const entry = {
    id: `guest_${Date.now()}`,
    name,
    photos,
    audio: {
      mime: sessionStorage.getItem('guestAudioMime'),
      data: sessionStorage.getItem('guestAudioData')
    },
    createdAt: new Date().toISOString()
  };
  const list = JSON.parse(localStorage.getItem('guestEntries') || '[]');
  list.push(entry);
  localStorage.setItem('guestEntries', JSON.stringify(list));
  alert('Saved locally! (No DB yet)');
